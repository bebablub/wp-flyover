  ...
  $(document).ready(function () {
    ...
    function validateAndPreviewFile($input, maxSizeMB = 20) {
      ...
      if (fileSize > maxSizeBytes) {
        const errorMsg = $(
          '<div class="file-error notice notice-error inline" style="margin: 5px 0;"><p>File size (' +
      ...
      // Show file info
      const fileInfo = $(
        '<div class="file-info notice notice-success inline" style="margin: 5px 0;"><p></p></div>'
      );
      fileInfo
        .find('p')
        .append($('<strong>').text('Selected: '))
        .append(
          document.createTextNode(fileName + ' (' + (fileSize / 1024 / 1024).toFixed(1) + 'MB)')
        );
    ...
    $('form').on('submit', function (e) {
      ...
      const $progressMsg = $(
        '<div class="fgpx-upload-progress notice notice-info" style="margin-top: 15px;"><p><strong>Uploading and processing GPX file...</strong><br>This may take a few moments for large files. Please do not close this page.</p></div>'
      );
    ...
    $('.fgpx-sync-gmedia-caption').on('click', async function (e) {
      ...
      const confirmRun = String(
        previewCfg.gmediaSyncConfirmRun ||
          'Sync Grand Media titles into WordPress image captions now? This runs across all matching image attachments by filename.'
      );
      const confirmOverwrite = String(
        previewCfg.gmediaSyncConfirmOverwrite ||
          'Overwrite existing captions? Click Cancel to keep existing captions and fill empty ones only.'
      );
      const batchSize =
        Number(previewCfg.gmediaSyncBatchSize) > 0 ? Number(previewCfg.gmediaSyncBatchSize) : 250;
      const pauseMs =
        Number(previewCfg.gmediaSyncPauseMs) >= 0 ? Number(previewCfg.gmediaSyncPauseMs) : 80;

      if (!gmediaAvailable) {
        showAdminNotice(
          gmediaReason ||
            'Grand Media is not available. Install and activate Grand Media before syncing captions.',
          'error'
        );
        return;
      }
      ...
      try {
        for (let guard = 0; guard < 10000; guard += 1) {
          ...
          updatedTotal = Number(
            payload.updated_total != null
              ? payload.updated_total
              : updatedTotal + Number(chunk.updated || 0)
          );
          ...
          const message =
            ...
            ' unmatched, ' +
            totals.duplicates +
            ' duplicate filename matches, ' +
            totals.errors +
            ' errors.';

          $link
            .text(hadPartial ? '⚠ Partial' : '✓ Synced')
            .css('color', hadPartial ? '#dba617' : '#46b450');
        ...
        throw {
          responseText: JSON.stringify({
            data: { message: 'GMedia caption sync stopped: too many batches.' },
          }),
        };
      } catch (xhr) {
    ...
    $('.fgpx-generate-preview').on('click', async function (e) {
      ...
      try {
        ...
        if (response && response.success) {
          ...
          const previewUrl =
            response.data && response.data.previewUrl ? response.data.previewUrl : '';
          const message =
            response.data && response.data.message
              ? String(response.data.message)
    ...
    $('.fgpx-clear-cache').on('click', function (e) {
      ...
      if (
        !confirm(
          'Clear cache for this track? The player will regenerate track data on next page load.'
        )
      ) {
        return;
      }
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.success) {
            $btn.text('✓ Cleared');
            showAdminNotice(
              'Playback statistics cleared successfully. The page will reload.',
              'success'
            );
            setTimeout(function () {
              location.reload();
            ...
            showAdminNotice(
              response.data && response.data.message
                ? response.data.message
                : 'Failed to clear playback statistics.',
              'error'
            );
          }
        },
    ...
    $('#fgpx-test-smart-keys').on('click', function (e) {
      ...
      const ajaxUrl = String(
        previewCfg.ajaxUrl || (typeof window.ajaxurl === 'string' ? window.ajaxurl : '') || ''
      );
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          rows.forEach(function (row) {
            ...
            const status =
              row && Number(row.status) > 0 ? ' (HTTP ' + Number(row.status) + ')' : '';
          ...
          const okCount = rows.filter(function (row) {
            return !!(row && row.ok);
          }).length;
    ...
    $('#doaction, #doaction2').on('click', async function (e) {
      ...
      if (action === 'fgpx_sync_gmedia_captions') {
        ...
        const selectedIds = $('input[name="post[]"]:checked')
          .map(function () {
        ...
        if (selectedIds.length === 0) {
          e.preventDefault();
          showAdminNotice(
            'Select at least one track before running GMedia caption sync.',
            'warning'
          );
          return;
        }
      ...
      if (targetIds.length > BULK_MAX_TRACKS) {
        ...
        showAdminNotice(
          'Safety limit applied: processing first ' +
            BULK_MAX_TRACKS +
            ' tracks now, leaving ' +
            (selectedIds.length - BULK_MAX_TRACKS) +
            ' for the next run.',
          'warning'
        );
      }
    ...
    $(document).on('click', '.fgpx-preview-custom-select', function (e) {
      ...
      frame.on('select', function () {
        ...
        const url =
          data.sizes && data.sizes.medium_large && data.sizes.medium_large.url
        ...
        if (url) {
          $box
            .find('.fgpx-preview-custom-thumb')
            .html(
              '<img src="' +
                String(url).replace(/"/g, '&quot;') +
                '" alt="Custom preview image" style="max-width:100%;height:auto;border:1px solid #ccd0d4;border-radius:6px" />'
            )
            .show();
        }
      });
    ...
    $(document).on('click', '.fgpx-preview-mode-save', function (e) {
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          const previewUrl =
            response.data && response.data.previewUrl ? String(response.data.previewUrl) : '';
          if (previewUrl) {
            upsertPreviewImage(previewUrl);
          } else {
            removePreviewImage();
          ...
          const source =
            response.data && response.data.source ? String(response.data.source) : 'none';
