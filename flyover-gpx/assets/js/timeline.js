...
(function () {
    ...
    return (urls || []).reduce(function (promise, url) {
      return promise.then(function () {
        return new Promise(function (resolve, reject) {
          ...
          if (
            [].slice.call(document.scripts).some(function (s) {
              return (s.src || '') === url;
            })
          ) {
            return resolve();
  ...
  function initTimeline(container, config) {
    ...
    var monthGroupingEnabled = !(
      monthGroupingRaw === false ||
      monthGroupingRaw === '0' ||
      monthGroupingRaw === 0 ||
      monthGroupingRaw === 'false'
    );
  ...
  function ajaxLoadTracks(state, params) {
    return new Promise(function (resolve, reject) {
      ...
      xhr.open(
        'GET',
        state.config.ajaxUrl + '?action=fgpx_timeline_tracks&' + buildQueryString(params)
      );
  ...
  function buildTrackCard(track, state) {
    ...
    if (track.previewUrl) {
      ...
      preview.innerHTML =
        '<svg class="timeline-placeholder-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>';
    }
    ...
    if (track.distanceKm) {
      appendMetaLine(
        i18n.distanceLabel || 'Distance',
        String(track.distanceKm) + ' ' + (i18n.distanceUnitKm || 'km')
      );
    }
    ...
    if (track.elevationGainLabel) {
      appendMetaLine(
        i18n.elevationGainLabel || 'Gain',
        String(track.elevationGainLabel) + ' ' + (i18n.elevationUnitM || 'm')
      );
    }
    if (track.dateLabel) {
  ...
  function openTrackModal(state, track) {
    ...
    closeBtn.setAttribute(
      'aria-label',
      (window.FGPXTimelineI18n && window.FGPXTimelineI18n.closeLabel) || 'Close'
    );
    ...
    ensurePlayerAssets(state.config)
      ...
      .catch(function (err) {
        ...
        p.textContent =
          (window.FGPXTimelineI18n && window.FGPXTimelineI18n.playerLoadFailed) ||
  ...
  function showTimelineError(container, error) {
    ...
    if (error && error.message) {
      ...
      if (error.message.indexOf('HTTP') === 0) {
        ...
        error.message.indexOf('NetworkError') >= 0
      ) {
        // Network error
  ...
  function showEmptyState(container) {
    ...
    message.textContent =
      i18n.noTracksMessage || 'Start by uploading your first GPX file to see tracks here.';
