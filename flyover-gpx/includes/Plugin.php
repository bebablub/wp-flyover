...
final class Plugin
{
    ...
    public function render_shortcode(array $atts = []): string
    {
        ...
        $defaults = [
            ...
            'arrows_enabled' => '',
            'arrows_km' => '',
            'speed_arrows_enabled' => '',
            'speed_arrows_threshold_low' => '',
            'speed_arrows_threshold_high' => '',
            'speed_arrows_color_low' => '',
            'speed_arrows_color_mid' => '',
            'speed_arrows_color_high' => '',
            'speed_arrows_spacing_low_km' => '',
            'speed_arrows_spacing_high_km' => '',
        ...
        if (!\in_array($mapStyle, ['default', 'url', 'inline'], true)) {
            $mapStyle = 'default';
        }
        ...
        if ($privacyAttr !== '') {
            if (\in_array($privacyAttr, ['1', 'true', 'yes', 'on'], true)) {
                $privacyOverride = true;
            } elseif (\in_array($privacyAttr, ['0', 'false', 'no', 'off'], true)) {
        ...
        $hudOverride = null;
        if ($hudAttr !== '') {
            if (\in_array($hudAttr, ['1', 'true', 'yes', 'on'], true)) {
                $hudOverride = true;
            } elseif (\in_array($hudAttr, ['0', 'false', 'no', 'off'], true)) {
                $hudOverride = false;
            }
        }
        ...
        if ($elevationColoringAttr !== '') {
            if (\in_array($elevationColoringAttr, ['1', 'true', 'yes', 'on'], true)) {
                $elevationColoringOverride = true;
            } elseif (\in_array($elevationColoringAttr, ['0', 'false', 'no', 'off'], true)) {
                $elevationColoringOverride = false;
            }
        }
        ...
        $resolveBooleanAttr = function ($attrRaw, $optionValue) {
            ...
            if ($attr !== '') {
                if (\in_array($attr, ['1', 'true', 'yes', 'on'], true)) {
                    return true;
                } elseif (\in_array($attr, ['0', 'false', 'no', 'off'], true)) {
                    return false;
                }
            }
        ...
        $speedArrowsEnabledFinal = $resolveBooleanAttr((string) ($atts['speed_arrows_enabled'] ?? ''), $options['fgpx_speed_arrows_enabled'] ?? '0');
        $speedArrowsThresholdLowFinal = $resolveFloatRangeAttr((string) ($atts['speed_arrows_threshold_low'] ?? ''), $options['fgpx_speed_arrows_threshold_low'] ?? '18', 1.0, 120.0);
        $speedArrowsThresholdHighFinal = $resolveFloatRangeAttr((string) ($atts['speed_arrows_threshold_high'] ?? ''), $options['fgpx_speed_arrows_threshold_high'] ?? '35', 2.0, 160.0);
        if ($speedArrowsThresholdHighFinal <= $speedArrowsThresholdLowFinal) {
            $speedArrowsThresholdHighFinal = min(160.0, $speedArrowsThresholdLowFinal + 1.0);
        }
        $speedArrowsColorLowFinal = $resolveColorAttr((string) ($atts['speed_arrows_color_low'] ?? ''), $options['fgpx_speed_arrows_color_low'] ?? '#ffd54f');
        $speedArrowsColorMidFinal = $resolveColorAttr((string) ($atts['speed_arrows_color_mid'] ?? ''), $options['fgpx_speed_arrows_color_mid'] ?? '#ff9800');
        $speedArrowsColorHighFinal = $resolveColorAttr((string) ($atts['speed_arrows_color_high'] ?? ''), $options['fgpx_speed_arrows_color_high'] ?? '#ff3d00');
        $speedArrowsSpacingLowKmFinal = $resolveFloatRangeAttr((string) ($atts['speed_arrows_spacing_low_km'] ?? ''), $options['fgpx_speed_arrows_spacing_low_km'] ?? '3.5', 0.3, 25.0);
        $speedArrowsSpacingHighKmFinal = $resolveFloatRangeAttr((string) ($atts['speed_arrows_spacing_high_km'] ?? ''), $options['fgpx_speed_arrows_spacing_high_km'] ?? '0.8', 0.1, 10.0);
        if ($speedArrowsSpacingHighKmFinal > $speedArrowsSpacingLowKmFinal) {
            $speedArrowsSpacingHighKmFinal = $speedArrowsSpacingLowKmFinal;
        }
        ...
        $localized = [
            ...
            'speedArrowsEnabled' => $speedArrowsEnabledFinal,
            'speedArrowsThresholdLow' => $speedArrowsThresholdLowFinal,
            'speedArrowsThresholdHigh' => $speedArrowsThresholdHighFinal,
            'speedArrowsColorLow' => $speedArrowsColorLowFinal,
            'speedArrowsColorMid' => $speedArrowsColorMidFinal,
            'speedArrowsColorHigh' => $speedArrowsColorHighFinal,
            'speedArrowsSpacingLowKm' => $speedArrowsSpacingLowKmFinal,
            'speedArrowsSpacingHighKm' => $speedArrowsSpacingHighKmFinal,
            ...
            'i18n' => [
                ...
                'speedArrowsUnavailable' => \esc_html__('Speed arrows are enabled but this track has no usable speed/timestamp data.', 'flyover-gpx'),
        ...
        if ($isFirstInstance) {
            ...
            \wp_add_inline_script(
                ...
                'speedArrowsEnabled:' . ($speedArrowsEnabledFinal ? 'true' : 'false') . ',' .
                'speedArrowsThresholdLow:' . \floatval($speedArrowsThresholdLowFinal) . ',' .
                'speedArrowsThresholdHigh:' . \floatval($speedArrowsThresholdHighFinal) . ',' .
                'speedArrowsColorLow:"' . \esc_js($speedArrowsColorLowFinal) . '",' .
                'speedArrowsColorMid:"' . \esc_js($speedArrowsColorMidFinal) . '",' .
                'speedArrowsColorHigh:"' . \esc_js($speedArrowsColorHighFinal) . '",' .
                'speedArrowsSpacingLowKm:' . \floatval($speedArrowsSpacingLowKmFinal) . ',' .
                'speedArrowsSpacingHighKm:' . \floatval($speedArrowsSpacingHighKmFinal) . ',' .
